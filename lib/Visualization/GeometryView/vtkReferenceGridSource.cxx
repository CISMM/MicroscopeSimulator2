/*=========================================================================
  ...
  Module:    $RCSfile: vtkReferenceGridSource.cxx,v $
...
vtkReferenceGridSource::vtkReferenceGridSource()
{
  this->XSpacing = 1.0;
  this->YSpacing = 1.0;
...
// Set the x-y spacing in the plane.
void vtkReferenceGridSource::SetSpacing(const double xR, const double yR)
{
  if ( xR != this->XSpacing || yR != this->YSpacing )
    {
    this->XSpacing = xR;
    this->YSpacing = yR;

    this->XSpacing = (this->XSpacing > 0.0 ? this->XSpacing : 1);
    this->YSpacing = (this->YSpacing > 0.0 ? this->YSpacing : 1);

    this->Modified();
    }
}

int vtkReferenceGridSource::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  //
  double distance1 = sqrt(vtkMath::Distance2BetweenPoints(this->Origin, this->Point1));
  double distance2 = sqrt(vtkMath::Distance2BetweenPoints(this->Origin, this->Point2));
  int XResolution = static_cast<int>(distance1 / this->XSpacing) + 1;
  int YResolution = static_cast<int>(distance2 / this->YSpacing) + 1;
  numPts = (XResolution+1) * (YResolution+1);
  numPolys = XResolution * YResolution;
  ...
  //
  for (numPts=0, i=0; i<(YResolution+1); i++)
    {
    if (i < YResolution) {
      tc[1] = (static_cast<double>(i)*this->YSpacing) / distance2;
    } else {
      tc[1] = 1.0;
    }
    for (j=0; j<(XResolution+1); j++)
      {
      if (j < XResolution) {
        tc[0] = (static_cast<double>(j)*this->XSpacing) / distance1;
      } else {
        tc[0] = 1.0;
      }
      for ( ii=0; ii < 3; ii++)
  ...
  for (i=0; i<YResolution; i++)
    ...
    for (j=0; j<XResolution; j++)
      {
      pts[0] = j + i*(XResolution+1);
      pts[1] = pts[0] + 1;
      pts[2] = pts[0] + XResolution + 2;
      pts[3] = pts[0] + XResolution + 1;
...
void vtkReferenceGridSource::SetNormal(double N[3])
{
...
// Set the normal to the plane. Will modify the Origin, Point1, and Point2
// instance variables as necessary (i.e., rotate the plane around its center).
void vtkReferenceGridSource::SetNormal(double nx, double ny, double nz)
{
  double n[3];
...
void vtkReferenceGridSource::SetCenter(double center[3])
{
...
// Set the center of the plane. Will modify the Origin, Point1, and Point2
// instance variables as necessary (i.e., translate the plane).
void vtkReferenceGridSource::SetCenter(double x, double y, double z)
{
  double center[3];
...
void vtkReferenceGridSource::SetPoint1(double pnt[3])
{
...
// modifies the normal and origin
void vtkReferenceGridSource::SetPoint2(double pnt[3])
{
...
void vtkReferenceGridSource::SetPoint1(double x, double y, double z)
{
  double pnt[3];
...
void vtkReferenceGridSource::SetPoint2(double x, double y, double z)
{
  double pnt[3];
...
void vtkReferenceGridSource::Push(double distance)
{
  int i;
...
int vtkReferenceGridSource::UpdatePlane(double v1[3], double v2[3])
{
  // set plane center
  for ( int i=0; i < 3; i++ )
    {
...
void vtkReferenceGridSource::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os,indent);

  os << indent << "X Spacing: " << this->XSpacing << "\n";
  os << indent << "Y Spacing: " << this->YSpacing << "\n";
