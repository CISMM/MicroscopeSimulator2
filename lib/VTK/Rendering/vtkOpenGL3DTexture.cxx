...
void vtkOpenGL3DTexture::LoadExtensions() {
...
#if 0
    // Truncate values below 1.0e-9 because values below that give problems
    // on some ATI hardware (speckling artifacts).
    for (int i = 0; i < size[0]*size[1]*size[2]; i++) {
      if (fabs(floatPtr[i]) < 1.0e-9) {
        floatPtr[i] = 0.0f;
      }
    }
#endif
